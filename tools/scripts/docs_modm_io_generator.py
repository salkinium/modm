...
def get_targets():
    ...
    raw_targets = builder.parser.find_option(":target").values
    ignored = list(filter(lambda d: "#" not in d, repopath("test/all/ignored.txt").read_text().strip().splitlines()))
    raw_targets = sorted(d for d in raw_targets if not any(d.startswith(i) for i in ignored))
...
def create_target(argument):
    ...
    try:
        ...
        first_board = next(m for m in modules if ":board:" in m)
